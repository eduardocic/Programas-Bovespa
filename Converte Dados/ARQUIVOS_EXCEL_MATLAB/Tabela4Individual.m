...
m = size(file.dado.DFIndResultadoPeriodo,2);
alfa = 0;
flag = 1;
for j=1:m
    posicao = file.dado.DFIndResultadoPeriodo(1,j);
    if(isnan(posicao) == 0 && alfa == 0)
       flag = j;
       alfa = 1;
    end
end
file.dado.DFIndResultadoPeriodo = file.dado.DFIndResultadoPeriodo(1:end,flag:end);
% -------------------------------------------------------------------------         
...
% Inserido 13/03/2016
    if(strcmp(str1, 'Despesas com Vendas'))
        if (strcmp(Numero{i},'3.04.01'))
            T4.DespesasComVendas  = file.dado.DFIndResultadoPeriodo(i,1);
        end
    end 

    if(strcmp(str1, 'Despesas Gerais e Administrativas'))
        if (strcmp(Numero{i},'3.04.02'))
            T4.DespesasGeraisEAdministrativas  = file.dado.DFIndResultadoPeriodo(i,1);
        end
    end 
% -------------------------------------------------------------------------    
