...
m = size(file.dado.DFIndAtivo,2);
alfa = 0;
flag = 1;
for j=1:m
    posicao = file.dado.DFIndAtivo(1,j);
    if(isnan(posicao) == 0 && alfa == 0)
       flag = j;
       alfa = 1;
    end
end
file.dado.DFIndAtivo = file.dado.DFIndAtivo(1:end,flag:end);
% -------------------------------------------------------------------------
...
for i=1:n
    ...
    if(strcmp(str1, 'Ativo Total'))
        if (strcmp(Numero{i},'1'))
            T2.AtivoTotal = file.dado.DFIndAtivo(i,1);
        end
    end    
