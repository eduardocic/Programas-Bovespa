function [o] = Tabela4Individual(o)
% 4.:  QUARTA TABELA
%     ---------------
% 
...
o.T4.texto = {'Receita de Vendas de Bens e/ou Serviços';
            ...
            'Lucro/Prejuízo do Período';                    % Alterado 16/03/2016
...
m = size(o.file.dado.DFIndResultadoPeriodo,2);
alfa = 0;
flag = 1;
for j=1:m
    posicao = o.file.dado.DFIndResultadoPeriodo(1,j);
    if(isnan(posicao) == 0 && alfa == 0)
       flag = j;
       alfa = 1;
    end
end
o.file.dado.DFIndResultadoPeriodo = o.file.dado.DFIndResultadoPeriodo(1:end,flag:end);
% -------------------------------------------------------------------------         



Numero = o.file.texto.DFIndResultadoPeriodo(3:end,1);
Texto  = o.file.texto.DFIndResultadoPeriodo(3:end,2);
n = max(size(Texto));
...
for i=1:n
    ...
    if(strcmp(str1, 'Receita de Venda de Bens e/ou Serviços'))
        if (strcmp(Numero{i},'3.01'))
            o.T4.ReceitaDeVendaDeBensEOUServicos  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end
    
    if(strcmp(str1, 'Custo dos Bens e/ou Serviços Vendidos'))
        if (strcmp(Numero{i},'3.02'))
            o.T4.CustoDosBensEOUServicosVendidos  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end 

    if(strcmp(str1, 'Resultado Bruto'))
        if (strcmp(Numero{i},'3.03'))
            o.T4.ResultadoBruto  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end
    
    if(strcmp(str1, 'Despesas/Receitas Operacionais'))
        if (strcmp(Numero{i},'3.04'))
            o.T4.DespesasReceitasOperacionais  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end 
...
% Inserido 13/03/2016
    if(strcmp(str1, 'Despesas com Vendas'))
        if (strcmp(Numero{i},'3.04.01'))
            o.T4.DespesasComVendas  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end 

    if(strcmp(str1, 'Despesas Gerais e Administrativas'))
        if (strcmp(Numero{i},'3.04.02'))
            o.T4.DespesasGeraisEAdministrativas  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end 
% -------------------------------------------------------------------------    
    ...
    if(strcmp(str1, 'Resultado Antes do Resultado Financeiro e dos Tributos'))
        if (strcmp(Numero{i},'3.05'))
            o.T4.ResultadoAntesDoResultadoFinanceiroEDosTributos = ...
                o.file.dado.DFIndResultadoPeriodo(i,1);
    ...
    if(strcmp(str1, 'Resultado Financeiro'))
        if (strcmp(Numero{i},'3.06'))
            o.T4.ResultadoFinanceiro  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end
    
    if(strcmp(str1, 'Resultado Antes dos Tributos sobre o Lucro'))
        if (strcmp(Numero{i},'3.07'))
            o.T4.ResultadoAntesDosTributosSobreOLucro  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end 
    ...
    if(strcmp(str1, 'Resultado Líquido das Operações Continuadas'))
        if (strcmp(Numero{i},'3.09'))
            o.T4.ResultadoLiquidoDasOperacoesContinuadas  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end
    
    if(strcmp(str1, 'Resultado Líquido de Operações Descontinuadas'))
        if (strcmp(Numero{i},'3.10'))
            o.T4.ResultadoLiquidoDeOperacoesDescontinuadas  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
    end
    
    if(strcmp(str1, 'Lucro/Prejuízo do Período'))   % Alterado 16/03/2016
        if (strcmp(Numero{i},'3.11'))
            o.T4.LucroPrejuizoDoPeriodo  = o.file.dado.DFIndResultadoPeriodo(i,1);
        end
        
% -------------------------------------------------------------------------        
% Inserido dia 14/03/2016
        if (strcmp(Numero{i+1}, '3.11.01'))
            o.T4.AtribuidoASociosDaEmpresaControladora  = o.file.dado.DFIndResultadoPeriodo(i+1,1);
            o.T4.AtribuidoASociosNaoControladora        = o.file.dado.DFIndResultadoPeriodo(i+2,1);
        else
            o.T4.AtribuidoASociosDaEmpresaControladora  = o.file.dado.DFIndResultadoPeriodo(i,1);
